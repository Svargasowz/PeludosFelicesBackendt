...
export const actualizarEstado = async (req, res) => {
    try {
        ...
        const [confirmacion] = await pool.query("UPDATE usuarios SET rol = 3 WHERE cedula=?", [cedula])
...
export const eliminarUsuario = async (req,res)=>{
    try{
        const {cedula } = req.params
        const [eliminar] = await pool.query("DELETE FROM usuarios WHERE cedula=?",[cedula])
